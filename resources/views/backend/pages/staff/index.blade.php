...
<div class="container mt-5">
    <h2 class="mb-4">Staff / Officer Management</h2>
    <button class="btn btn-primary mb-3" id="add-staff">Add New Staff / Officer</button>
...
<script>
    $(document).ready(function() {
        ...
        // Global arrays for departments and designations
        let departments = @json($departments);
        let designations = @json($designations);
        ...
        function populateDesignationDropdown(selectedId = null) {
            ...
            designations.forEach(designation => {
                designationDropdown.append(
                    `<option value="${designation.id}" ${designation.id == selectedId ? 'selected' : ''}>${designation.name}</option>`
                );
            });
        }
        ...
        $('#add-staff').click(function() {
            $('#staff-form')[0].reset();
            $('#staff_id').val('');
            populateDepartmentDropdown(); // Populate department dropdown
        ...
        $('#staff-form').submit(function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    iziToast.success({
                        message: $('#staff_id').val() ? 'Staff updated successfully!' : 'Staff created successfully!',
                        position: 'topRight',
                    });
        ...
        $('#staff-table').on('click', '.edit-btn', function() {
            ...
            $.get('{{ route("staff.edit", "") }}/' + id, function(data) {
                ...
                populateDepartmentDropdown(data.department_id); // Populate department dropdown
                populateDesignationDropdown(data.designation_id); // Populate designation dropdown
